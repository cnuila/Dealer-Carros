...
class Principal extends React.Component {
    ...
    constructor(props) {
        super(props)
        this.state = {
            estados: [{ estado: 'Disponible', selected: true, color: "text-green-500" },
    ...
    clicEstadoCarro(estadoEnviado) {
        ...
        estados.map(estado => {
            ...
            if (estado.estado === estadoEnviado) {
    ...
    render() {
        ...
        botonesEstados = estados.map(boton => {
            return (
                <div>
                    <Estado estado={boton.estado} selected={boton.selected} color={boton.color} clicEstadoCarro={this.clicEstadoCarro}></Estado>
                </div>
            )
        });
        return (
            <div className="bg-gray-100">
                ...
                {/*fin del navbar*/}
                <div className="grid grid-cols-7 place-items-center m-8">
                    {botonesEstados}
                    <div></div>
                    <div></div>
                    <button className="focus:outline-none hover:bg-gray-300 text-gray-900 text-sm py-2 px-4 rounded-lg inline-flex items-center w-40">
                        <svg className="h-5 w-5 fill-current text-gray-900" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                        ...
                        <span className="flex-1">Mostrar filtros</span>
                    </button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 place-items-center mt-8 mx-4 md:mx-8">
